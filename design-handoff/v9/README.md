# Handoff: The Dreamer — 4-Tone Vector ROMpler GUI (v9, per GUI_SPEC.md)
...
6. **FX** — three rows separated by 1px dividers, each: name (30px), < > arrows + type button (44px, click = menu), 4 knobs Ø30 at 36px
   ...
   DELAY (DIGITAL/TAPE/PONG): TIME, FB, PARAM, MIX + **SYNC btn + LED** — when lit the
   TIME knob's pointer turns yellow and its label shows the tempo division (4/1…1/32,
   same table as LFO sync); dly_sync is an APVTS parameter.
