# Handoff: The Dreamer — 4-Tone Vector ROMpler GUI (v7, per GUI_SPEC.md)
...
meter; **MASTER volume knob (Ø40)** and POWER LED right.
...
2. **TONE EDIT <X>** (X = selected tone letter, `#ff5b6e`):
   ...
   - Rows: **LFO1** and **LFO2** — each RATE + DEPTH knobs Ø26, SYNC btn + LED, DEST
...
3. **FILTERS** — two strips: type LCD (click = menu; `<` `>` steppers) + 3 knobs Ø36:
   ...
   NL3 N+LP, FORMANT, ALLPASS, DREAMPLN. Next to SER/PAR: **BAL 1↔2 knob Ø26
...
6. **FX** — three rows separated by 1px dividers, each: name, type button, 3 knobs Ø32,
   LED + on/off button:
   MOD (CHORUS/FLANGER/PHASER/ENSEMBLE): RATE, DEPTH, MIX. DELAY (DIGITAL/TAPE/PONG): TIME, FB,
