...
## STATE (updated 2026-07-21, RC 2.4.1 — GUI preset-load fix on top of v15 GUI + DSP align; validator PASS + pluginval 8, deployed)

> History (0.1.0 … 2.3.0) lives in CHANGELOG.md. This block is CURRENT-ONLY.

| Item | Value |
|---|---|
| Deployed version | **2.4.1 RC** — binary literal `2.4.1`, `The Dreamer.vst3` at C:\the-dreamer\ AND \\VBOXSVR\vagrant\ (share root), build↔local↔share SHA-identical. Validator PASS all stages (dsp 10/10, staging/bundle/gui load+screenshot/deploy/host pluginval 8). **2.4.1 = GUI-only preset-load fix vs 2.4.0** → NO param change since 2.4.0 (just reload the instance / restart Cubase). **Coming from ≤2.3.0 needs a FULL RE-SCAN** (2.4.0 param-list change). `juce_add_plugin VERSION ${PROJECT_VERSION}`. |
