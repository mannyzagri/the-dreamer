...
## STATE (updated 2026-07-21, RC 2.5.0 — v16 GUI (adopts the integration contract) + Global LFO 2; validator PASS + pluginval 8, deployed)

> History (0.1.0 … 2.4.1) lives in CHANGELOG.md. This block is CURRENT-ONLY.

| Item | Value |
|---|---|
| Deployed version | **2.5.0 RC** — binary literal `2.5.0`, `The Dreamer.vst3` at C:\the-dreamer\ AND \\VBOXSVR\vagrant\ (share root), build↔local↔share SHA-identical. Validator PASS all stages (dsp 10/10, staging/bundle/gui load+screenshot/deploy/host pluginval 8). ⚠ **PARAM-LIST CHANGE → Cubase FULL RE-SCAN** (added Global LFO 2 + matrix sources 5→7). `juce_add_plugin VERSION ${PROJECT_VERSION}`. |
