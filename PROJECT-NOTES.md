...
## STATE (updated 2026-07-22, RC 2.5.2 — TD-001 0dBFS-noise fix RELEASED+DEPLOYED; validator dsp 11/11 + bundle/deploy/pluginval 8; ear pass pending)

> History (0.1.0 … 2.5.1) lives in CHANGELOG.md. This block is CURRENT-ONLY.

| Item | Value |
|---|---|
| Deployed version | **2.5.2 RC** — binary literal `2.5.2`, `The Dreamer.vst3` at C:\the-dreamer\ AND \\VBOXSVR\vagrant\ (share root), build↔local↔share SHA-identical. Release tool verdict: **RE-SCAN REQUIRED** (moduleinfo changed) + remove/re-add instance (VST3s don't hot-reload); header stamp must read 2.5.2. pluginval 8 SUCCESS. **EAR GATE PENDING:** SOLINA FIELDS @44.1 k, play + idle 60 s → must stay clean. `juce_add_plugin VERSION ${PROJECT_VERSION}`. |
