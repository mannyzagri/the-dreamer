...
## STATE (updated 2026-07-22, RC 2.5.1 — loop-tuning regression fix (v3 roots→220); validator dsp 10/10 + pluginval 8, deployed)

> History (0.1.0 … 2.5.0) lives in CHANGELOG.md. This block is CURRENT-ONLY.

| Item | Value |
|---|---|
| Deployed version | **2.5.1 RC** — binary literal `2.5.1`, `The Dreamer.vst3` at C:\the-dreamer\ AND \\VBOXSVR\vagrant\ (share root), build↔local↔share SHA-identical. Validator PASS (dsp 10/10, staging/bundle/gui/deploy/host pluginval 8). **2.5.1 = bank-tuning fix vs 2.5.0** → NO param change (just reload / restart Cubase). **Coming from ≤2.4.x needs a FULL RE-SCAN** (2.5.0 added Global LFO 2 + 2 matrix sources). `juce_add_plugin VERSION ${PROJECT_VERSION}`. |
