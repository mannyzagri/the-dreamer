...
## STATE (updated 2026-07-22, TD-001 0dBFS-noise ROOT-CAUSED+FIXED on branch fix/td-001-noise; validator dsp 11/11; release 2.5.2 pending user go)

> History (0.1.0 … 2.5.1) lives in CHANGELOG.md. This block is CURRENT-ONLY.

| Item | Value |
|---|---|
| TD-001 (0 dBFS noise) | ✅ **ROOT-CAUSED + FIXED, merged main `0c52c12`, awaiting release** — build\ Release REBUILT with the fix 2026-07-22 23:11 (stamp still 2.5.1 until release.ps1 bumps; **NOT deployed** — share/local still carry the bug). Docs: RELEASE-NOTES.md (user-facing) + validation/TD-001-REPORT.md (full incident report). (See CHANGELOG 2.5.2 entry): one-past-end heap read in Ensemble/Dimension/Rotary `read()` (float wrap rounds pos to exactly len_), deterministic on SOLINA FIELDS ~14 s @44.1 k, reverb trapped the burst, limiter pinned it at ceiling. + D13 pure-NaN blindness fixed (fmax swallows NaN; finite ≠ NaN per user), flushFx/prepareToPlay/StereoWidth reset completeness. New harness `fx_read_bounds` (validator 11/11); end-to-end 2× vst3-probe suite clean. Soak tool: C:\code-bank\tools\vst3-probe (generalized from tools/hostprobe). Tracer `DREAMER_TD001_TRACE` compiled-out in processBlock — strip after 2.5.2 soaks green. Remaining from mac triage: TASK 2 (GUI resize pipeline check), TASK 3 (library v4, blocked on zip). |
| Deployed version | **2.5.1 RC** — binary literal `2.5.1`, `The Dreamer.vst3` at C:\the-dreamer\ AND \\VBOXSVR\vagrant\ (share root), build↔local↔share SHA-identical. **2.5.1 STILL CARRIES TD-001** — 2.5.2 fixes it (no param change → reload instance, no re-scan). **Coming from ≤2.4.x needs a FULL RE-SCAN** (2.5.0 added Global LFO 2 + 2 matrix sources). `juce_add_plugin VERSION ${PROJECT_VERSION}`. |
