...
## STATE (updated 2026-07-19, RC 2.0.1 — v13 GUI resize/overflow fixes on top of 2.0.0, validator PASS + pluginval 8)

> History (0.1.0 … 2.0.0) lives in CHANGELOG.md. This block is CURRENT-ONLY.

| Item | Value |
|---|---|
| Deployed version | **2.0.1 RC (v13 faceplate resize/overflow fixed)** — binary literal `2.0.1`, `The Dreamer.vst3` at C:\the-dreamer\ AND \\VBOXSVR\vagrant\ (share root), build↔local↔share SHA-identical. Validator PASS (bundle/gui/deploy/host pluginval 8; dsp unchanged from 2.0.0). **GUI-only patch: NO param-list change vs 2.0.0** → just reload the instance / restart Cubase (no re-scan). NOTE: coming from 1.2.0 or earlier still needs the 2.0.0 FULL RE-SCAN (114→218 waves + 26 params). |
| 2.0.1 fix | Three Cubase resize bugs (all one root cause) + FX overflow, GUI-only. Removed the leftover v8 prototype resize grip (it scaled content via `uiScale` independent of the window → buttons overflow / frame grows only on fold / layout scales in fixed frame); `fit()` now derives scale ONLY from window size; host window border is the sole resize path. Fixed FX group overflow (row2 `1fr`/minmax(auto) inflated by the DELAY SYNC column pushed on/off buttons + UTIL past the panel edge) by tightening FX knob pitch to the master (gap 5→3, wrappers 36→32/40→36). Confirmed against the design-track double-check doc's new "Window resize / three bugs" section (design-handoff/v13 doc-only revision synced; master PNG identical). |
