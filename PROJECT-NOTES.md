...
## STATE (updated 2026-07-20, RC 2.2.0 — UX polish round D1–D14 + D11, validator dsp 10/10 + pluginval 8, deployed)

> History (0.1.0 … 2.1.2) lives in CHANGELOG.md. This block is CURRENT-ONLY.

| Item | Value |
|---|---|
| Deployed version | **2.2.0 RC** — binary literal `2.2.0`, `The Dreamer.vst3` at C:\the-dreamer\ AND \\VBOXSVR\vagrant\ (share root), build↔local↔share SHA-identical. Validator PASS all stages (dsp 10/10, staging/bundle/gui/deploy/host pluginval 8). ⚠ **PARAM-LIST CHANGE (~276→~296) → Cubase FULL RE-SCAN.** CMakeLists now sets `juce_add_plugin VERSION ${PROJECT_VERSION}` (PE tracks version, [[juce-pe-version-resource-gotcha]]). |
| UX round (v2.2.0) | UX_DSP_TASKS D1–D14 + D11 done; GUI hand-off = design-handoff/UX_ROUND_HANDOFF.md. New params: semi_[t], detune_voices/detune_amount_[t], g_env_a/d/s/r, g_cutoff, g_res, g_octave, limiter_on; tvf_kf now **bipolar** (−100..+100, presets migrated). Env A/D/R now log 1 ms–10 s with real-unit display (preset times migrated). D2 live env-rate. New processor API for the GUI: getScopeData (D3 FFT tap), getLimiterGR (D12 LED), panic (D13), resetToInit/program-0 INIT (D4), midiLearn* (D6), user-preset save/rename/delete/load + getUserPresetList (D14). D11 = per-wave **playback** loudness gain (bank untouched, −14 dBFS RMS, tools/bake_wave_norm.cpp → dsp/glue/WaveNormTable.h). **D15/D16 = next round.** |
| Bank / library | **218 waves** = 78 cycles (0-77) + 130 loops (78-207, **dreamer-library-v3**; per-family tags Pad/Airy/Vox/Ether/FM/Wind/Metal/Morph) + 10 HITs (208-217, "Hit"). Names/order LOCKED across v1/v2/v3 (indices = preset refs = wave-choice param). LoopBankData.h ≈102 MB (LFS) / 28 MB int16; ShotBankData.h ≈0.8 MB. WAV source assets/loops (130) + assets/shots (10); bake via tools/bake_*_header.cpp (no python). Per-loop tuning: dsp/bank/LoopRoots.h (130, from loop_roots.json; METAL/metal-morphs 220). ⚠ **v3 shipped no loop_roots.json → LoopRoots.h holds the v2-measured roots**; if v3 re-tuned the samples, a v3 loop_roots.json is needed to re-measure. 12-bit low-nibble invariant asserted. |
...
| Pending | **user Cubase ear+eye pass on 2.2.0** (FULL RE-SCAN first). Listen: (D2) cut a long RELEASE while a note rings → tail shortens immediately; (D1) LCD shows ms/s; (D9) DETUNE VOICES 2–4 widens without wander; (D10) −KF darkens up the keyboard; (D5) GLOBAL offsets shift all tones; (D7/D8) SEMI + global octave; (D11) browse waves → loudness now even (may feel ~10 dB quieter overall — say if too quiet, target is a one-liner); (D4) program 0 = INIT; (D12) limiter on/off; (D6/D14/D3/D13) need GUI wiring to exercise. Then GUI-Claude wires UX_ROUND_HANDOFF.md. Next DSP round: **D15** (PLAY MODE all wave types + granular LOOP RATE — new params, re-scan) + **D16** (naming). Still open: v3 loop_roots.json if re-tuned; FX-PARAM matrix wiring; exact E-MU Z-plane coefficients. |
| Git | main @ v2.2.0 = merge `980b0ca` (UX round branch port/ux-d1-d2-env → merge --no-ff → pushed; + this STATE touch-up). Branch → merge --no-ff → push per commit. Sound library tracked via **Git LFS** (`.gitattributes`: `assets/{loops,shots}/*.wav`, `dsp/bank/{LoopBankData,ShotBankData}.h`) because LoopBankData.h ~102 MB > GitHub's 100 MB limit. Fresh clone needs `git lfs pull` before building. Share the-dreamer-src\ export = working-tree robocopy (NOT `git archive` — that exports LFS pointer stubs). |
