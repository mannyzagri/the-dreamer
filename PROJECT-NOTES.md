...
## STATE (updated 2026-07-21, RC 2.4.0 — v15 production GUI integrated + DSP aligned; validator staging/gui/host PASS + pluginval 8, deployed)

> History (0.1.0 … 2.3.0) lives in CHANGELOG.md. This block is CURRENT-ONLY.

| Item | Value |
|---|---|
| Deployed version | **2.4.0 RC** — binary literal `2.4.0`, `The Dreamer.vst3` at C:\the-dreamer\ AND \\VBOXSVR\vagrant\ (share root), build↔local↔share SHA-identical. Validator PASS all stages (dsp 10/10, staging/bundle/gui load+screenshot/deploy/host pluginval 8). ⚠ **PARAM-LIST CHANGE → Cubase FULL RE-SCAN.** `juce_add_plugin VERSION ${PROJECT_VERSION}` (PE tracks version). |
